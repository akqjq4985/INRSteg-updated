...
class PointCloud(Dataset):
    def __init__(self, pointcloud_path, on_surface_points, keep_aspect_ratio=True):
        ...
        #     # Resize(sidelength, interpolation=InterpolationMode.NEAREST, antialias=True), # interpolation=InterpolationMode.NEAREST, 
        #     # Resize(sidelength),
        #     ToTensor(),
        #     Normalize(torch.Tensor([0.5]), torch.Tensor([0.5]))
        #     # Normalize(mean=[0.485, 0.456, 0.406], std=[0.229, 0.224, 0.225])
        # ])        
        # self.normals = self.transform(self.normals)
        # print(self.normals.shape)
        # self.normals = self.normals.squeeze()
    ...
    def __getitem__(self, idx):
        ...
        # print(on_surface_normals.shape)
        # print(off_surface_normals.shape)
...
class Video(Dataset):
    def __init__(self, path_to_video):
        ...
        for i in range(len(self.selected_frames)):
            self.selected_frames[i] = cv2.resize(self.selected_frames[i], (128,128))
            
        self.vid = self.selected_frames
...
class ImageNet(Dataset):
    def __init__(self):
        super().__init__()
        self.root = '../data/imagenet'
        self.img_channels = 3
        list_classes = os.listdir(self.root)
        self.fnames = []
        for cl in list_classes:
            temp = glob.glob(os.path.join(self.root, cl)+"/*.JPEG")
            self.fnames = self.fnames + temp
        # self.fnames = self.fnames[-3:]
    def __len__(self):
...
class DummyImplicitAudioWrapper(torch.utils.data.Dataset):
    def __init__(self, dataset, pad):
        ...
        m = torch.nn.ZeroPad2d((0,pad,0,0))
        self.grid = m(self.grid)
...
class AudioFile(Dataset):
    def __init__(self, filename):
        ...
        print("Length: %d" % self.file_length)
...
class ImagenetImplicit2DWrapper(torch.utils.data.Dataset):
    def __init__(self, dataset, sidelength=None, compute_diff=None):
    ...
    def __getitem__(self, idx):
        ...
        if self.compute_diff == 'gradients':
            ...
            grady = scipy.ndimage.sobel(img.numpy(), axis=2).squeeze(0)[..., None]
            laplace = scipy.ndimage.laplace(img.numpy()).squeeze(0)[..., None]
        img_channels = len(img_f.mode)
        if img_channels != 3:
            path = 'none'
        else:
            path = path.split("_")[-1].split(".")[0]
        img = img.permute(1, 2, 0).view(-1, img_channels)
        in_dict = {'idx': idx, 'coords': self.mgrid, 'path':path}
        gt_dict = {'img': img}
...
class DummyImplicit2DWrapper(torch.utils.data.Dataset):
    def __init__(self, dataset, sidelength=None, compute_diff=None):
